...
class IEEPISStatsOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $assignedEquipment = Equipment::where(
            "accountability_status",
            "assigned",
        )
            ->orWhere("accountability_status", "Normal")
            ->count();
        $unassigned = Equipment::where(
            "accountability_status",
            "unassigned",
        )->count();
        $nonFunctional = Equipment::where("is_functional", false)->count();
        $forDisposal = Equipment::where(
            "accountability_status",
            "For Disposal",
        )->count();
        ...
        return [
            Stat::make("Total ICT Equipment", $totalEquipment)
                ->description(
                    "{$dcpEquipment} DCP items · {$totalSchools} schools",
                )
            ...
            Stat::make("Assigned Equipment", $assignedEquipment)
                ->description(
                    $totalEquipment > 0
                        ? round(($assignedEquipment / $totalEquipment) * 100) .
                            "% utilization rate"
                        : "No equipment",
                )
            ...
            Stat::make(
                "Non-Functional / For Disposal",
                $nonFunctional + $forDisposal,
            )
                ->description(
                    "{$nonFunctional} non-functional · {$forDisposal} for disposal",
                )
            ...
            Stat::make("Total Schools", $allSchools)
            ...
            Stat::make("Open Tickets", $openTickets)
                ->description(
                    $openTickets > 0 ? "Requires attention" : "All clear!",
                )
                ->descriptionIcon(
                    $openTickets > 0
