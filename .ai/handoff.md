...
- Refactored `OfflineSyncController` for the QR Scanner: Added `ScanResolveRequest` and `ScanSyncRequest` for validation. Fixed payload parsing mismatch so it now supports the `IEEPIS|property_no|...` format generated by `Equipment` model. Applied `Gate::authorize('view')` checks for resolved records, and removed N+1 queries from bulk syncing.
